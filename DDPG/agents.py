...
class DDPG(Agent):
    ...
    def schedule_hyperparameters(self, timestep: int, max_timesteps: int):
        ...
        **YOU MAY IMPLEMENT THIS FUNCTION FOR Q5**
        ...
        """
        # Example of scheduling the noise standard deviation for exploration
        exploration_fraction = 0.2
        start_std = 0.1
        end_std = 0.01

        if timestep < max_timesteps * exploration_fraction:
            current_std = start_std - (timestep / (max_timesteps * exploration_fraction)) * (start_std - end_std)
        else:
            current_std = end_std

        self.noise.std = current_std * torch.ones(self.action_space.shape[0])
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q4**
        ...
        q_loss = F.mse_loss(current_q_values, targets)
