    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(timestep: int, max_timestep: int):
            ### PUT YOUR CODE HERE ###
            exploration_progress = min(timestep / (self.exploration_fraction * max_timestep), 1)
            epsilon = self.epsilon_start - (self.epsilon_start - self.epsilon_min) * exploration_progress
            return epsilon


        def epsilon_exponential_decay(timestep: int, max_timestep: int):
            ### PUT YOUR CODE HERE ###
            epsilon = max(self.epsilon_start * (self.epsilon_exponential_decay_factor ** (timestep / max_timestep)), self.epsilon_min)
            return epsilon
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if not explore or np.random.rand() > self.epsilon:
            with torch.no_grad():
    ...
    ** YOU NEED TO IMPLEMENT THE FUNCTIONS IN THIS CLASS **
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        action_probs = self.policy(obs_tensor).detach().numpy().squeeze()
    ...
    def update(
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        log_probs = torch.log(torch.sum(self.policy(observations_tensor) * torch.nn.functional.one_hot(actions_tensor, self.action_space.n), dim=1))
        p_loss = -torch.mean(log_probs * torch.tensor(discounted_rewards, dtype=torch.float32))
